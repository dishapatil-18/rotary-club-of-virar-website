...
$navItems = [
    ...
    'admins' => ['label' => 'Admin Accounts', 'icon' => 'shield', 'url' => 'admins_management.php', 'section' => 'Administration'],
    'rotary-years' => ['label' => 'Rotary Years', 'icon' => 'calendar', 'url' => 'rotary_years.php', 'section' => 'Administration'],
    'leadership' => ['label' => 'Leadership', 'icon' => 'users', 'url' => 'leadership_transfer.php', 'section' => 'Administration'],
    'site-content' => ['label' => 'Website Content', 'icon' => 'edit', 'url' => 'site_content.php', 'section' => 'Administration'],
    'contact-messages' => ['label' => 'Contact Messages', 'icon' => 'mail', 'url' => 'contact_messages.php', 'section' => 'Administration'],
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-logo">
        <div class="logo-icon flex-center" style="background:rgba(255,192,0,0.12); border:none;">
            <img src="../assets/uploads/Logo/rotary-icon.png" alt="Rotary Logo" style="width:40px;height:40px;border-radius:10px;object-fit:cover;" onerror="this.style.display='none';document.getElementById('logoFallback').style.display='flex'">
    ...
    <div style="padding:12px 16px 4px;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;align-items:center;gap:10px;flex-shrink:0;">
        <img src="<?= htmlspecialchars($_SESSION['admin_photo'] ?? '../assets/uploads/admins/Admin.jpg') ?>"
        ...
        <div style="flex:1;min-width:0;">
            <div style="font-size:12px;font-weight:600;color:rgba(255,255,255,0.85);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($_SESSION['admin_name'] ?? 'Admin') ?></div>
            <div style="font-size:10px;color:rgba(255,255,255,0.4);"><?= htmlspecialchars($_SESSION['admin_role'] ?? 'Administrator') ?></div>
        </div>
    </div>
    ...
    <nav class="sidebar-nav">
        <?php foreach ($sectionOrder as $section): $hasItems = false; ?>
            <?php foreach ($navItems as $id => $item): ?>
                <?php if ($item['section'] === $section): $hasItems = true; break; endif; ?>
            <?php endforeach; ?>
            <?php if ($hasItems): ?>
